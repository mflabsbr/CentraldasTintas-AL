...
export default function PriceConsultationApp() {
  ...
  // Substituir a função loadCompanyData inteira por esta versão que trata o novo formato:
  ...
  const loadCompanyData = async () => {
    try {
      ...
      // Verificar se é um objeto direto (novo formato)
      if (data && typeof data === "object" && !Array.isArray(data) && data.Fantasia) {
        if (data.Fantasia !== "--------") {
          setCompanyData(data.Fantasia)
        }
      }
      ...
      if (Array.isArray(data)) {
        const mappedProducts = data
          .map((item) => ({
            ...
            naoVender: item["NaoVender"] || "0",
            desativado: item["Desativado"] || "0",
          }))
          .filter((p) => {
            // Filtros de validação básica
            if (!p.name || p.name.includes("---")) return false
            if (isNaN(p.price)) return false

            // Filtro de preço zero
            if (p.price === 0.0) return false

            // Filtros de disponibilidade
            if (p.naoVender === "1") return false
            if (p.desativado === "1") return false
    ...
    return (
      <div className="h-screen bg-gradient-to-br from-purple-600 to-purple-800 flex flex-col">
        <div className="flex-1 flex flex-col justify-center p-4 py-4 pb-20">
          <div className="w-full max-w-md mx-auto py-0 px-0 my-0 h-80">
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-bold text-white mb-3">Consulta Preço</h1>

              {/* Campo de pesquisa no cabeçalho */}
              <form onSubmit={handleManualSearch} className="mb-6">
                <div className="relative mx-0 px-0 py-0 my-0">
                  <Input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Digite o código ou nome do produto"
                    className="text-base p-4 bg-white rounded-xl border-0 focus:ring-2 focus:ring-yellow-400 transition-all pr-12 pb-4 my-0 mx-1.5 pl-3 w-[97%]"
                  />
                  <Button
                    type="submit"
                    size="icon"
                    className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-yellow-400 hover:bg-yellow-500 text-purple-800 rounded-lg mx-0 my-0 py-0 px-0"
                  >
              ...
              <Button
                ...
                disabled={loading}
                className="bg-yellow-400 hover:bg-yellow-500 text-purple-800 font-bold text-base rounded-xl shadow-lg transition-all duration-200 active:scale-95 px-0 py-0 mr-0 ml-0 mb-4 mt-0 w-full"
              >
